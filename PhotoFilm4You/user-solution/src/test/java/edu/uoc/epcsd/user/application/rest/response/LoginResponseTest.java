...
class LoginResponseTest {
    ...
    @Test
    void equalsHandlesNullDifferentTypeAndSelf() {
        LoginResponse response = new LoginResponse(3L, "Tester", "t@example.com", "555-555-555", "token-value");

        assertThat(response.equals(null)).isFalse();
        assertThat(response.equals("not-a-login-response")).isFalse();
        assertThat(response.equals(response)).isTrue();
    }

    @Test
    void equalsAndHashCodeDetectDifferentFields() {
        LoginResponse base = new LoginResponse(4L, "Tester", "t@example.com", "555-555-555", "token-value");

        assertThat(base)
                .isNotEqualTo(new LoginResponse(4L, "Tester", "t@example.com", "555-555-555", "another-token"))
                .doesNotHaveSameHashCodeAs(new LoginResponse(5L, "Tester", "t@example.com", "555-555-555", "token-value"));
    }
}
